...
- **Transient-error recovery**: precisely classifies Codex's canonical “error occurred while processing your request” and status-less internal-server-error messages as retryable `SERVER` failures. DSH's default policy performs durable backoff retries while retaining the provider request ID; other unknown `CODEX_ERROR` failures are not retried blindly.
